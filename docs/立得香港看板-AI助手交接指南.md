...
| 名称 | 做什么 | 状态 |
|------|--------|------|
| **AI 叙事 Phase 1 · 工作台环比起承** | 页头下称呼 + 两段正文（不展示「起／承」字），模板文案，**无 LLM** | ✅ 2026-07-18 已发版 |
...
| 项 | 定稿 |
|----|------|
| 挂载 | `DashboardPage`：AppShell 标题下、图表区之上（Admin 预警摘要条之下） |
| 结构 | 称呼一行 + 起一段 + 承一段（**不展示「起／承」字样**，仅内部分段） |
| 称呼 | `{displayName}，您好。`（缺名则「您好。」） |
| 起 | 上月 vs 上上月；**显式月份**；总额环比 + 日均环比（`monthCompare`） |
