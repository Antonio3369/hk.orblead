...
function TruckSvg() {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 198 93" className="truck-svg" aria-hidden>
      <path
        strokeWidth="3"
        stroke="#282828"
        fill="#F83D3D"
...
function TireSvg() {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 30 30" className="tire-svg" aria-hidden>
...
export function PageLoader({ fullPage = false, block = false, compact = false }: PageLoaderProps) {
  const mode = fullPage ? "full" : compact ? "compact" : block ? "block" : "panel";

  return (
    <div className={`page-loader page-loader--${mode}`} role="status" aria-live="polite" aria-label="載入中">
      <div className="page-loader__inner">
        ...
        <p className="page-loader__label">載入中…</p>
      </div>
    </div>
  );
}