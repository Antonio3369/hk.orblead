...
export function TigerTeamPage({
  onBack,
  onOpenSales,
  apiPath = "/tiger-team",
  title = "飛虎隊",
  emptyHint = "暫無啟用中的銷售帳號，請先在後臺管理新增。",
}: TigerTeamPageProps) {
  ...
  useEffect(() => {
    setLoading(true);
    api<{ sales: TigerTeamSalesRow[] }>(`${apiPath}?sort=${sort}`)
  ...
  const sortHint =
    SORT_BUTTONS.find((b) => b.value === sort)?.label ?? "上月交易";

  return (
    <AppShell
      title={title}
      subtitle={`${BRAND.companyName} · ${sales.length} 位銷售 · 新沉默 ${totalSilent} 家 · ${totalUnread} 條未跟進預警`}
      onBack={onBack}
    >
      <section className="panel">
        <p className="panel-desc panel-desc-tight">
          默認按<strong>上月交易額</strong>排名。切換下方按鈕，按新沉默、下跌中、上漲或預警跟進重新排序。
        </p>
            ...
            <table className="data-table">
              <thead>
                <tr>
                  ...
                  <th>登入名</th>
                  <th>活躍/歸屬</th>
                  <th className={sort === "lastMonthAmount" ? "sort-col-active" : undefined}>
                    上月交易額
                  </th>
                  <th>本月 MTD</th>
                  <th>日均環比</th>
                  ...
                  <th className={sort === "unreadAlerts" ? "sort-col-active" : undefined}>
                    未跟進預警
                  </th>
              ...
              <tbody>
                {sales.map((s, index) => (
                  ...
                  >
                    ...
                    <td
                      className={sort === "newSilent" ? "sort-col-active" : undefined}
                      data-label="新沉默"
                    >
                      <CountBadge count={s.newSilentCount} tone="rose" />
                    </td>
                    <td
                      className={sort === "declining" ? "sort-col-active" : undefined}
                      data-label="下跌中"
                    >
                      <CountBadge count={s.decliningCount} tone="amber" />
                    </td>
                    <td
                      className={sort === "rising" ? "sort-col-active" : undefined}
                      data-label="上漲"
                    >
                      <CountBadge count={s.risingCount} tone="green" />
                    </td>
                    <td
                      className={sort === "unreadAlerts" ? "sort-col-active" : undefined}
                      data-label="未跟進預警"
                    >
                      {s.unreadAlerts > 0 ? (
